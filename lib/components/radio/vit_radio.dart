...
///
/// The [VitRadio] can be rendered as a standalone radio button or as part of a list item
/// with title, subtitle, and configurable positioning. Use [VitRadioGroup] to easily
...
///   options: const [
///     VitRadioOption(value: 'small', title: 'Small'),
///     VitRadioOption(value: 'medium', title: 'Medium'),
///     VitRadioOption(value: 'large', title: 'Large'),
///   ],
/// )
/// ```
///
...
enum VitRadioPosition {
  left,
  right,
}

class VitRadio<T> extends StatelessWidget {
  ...
  /// of the list item. Defaults to [VitRadioPosition.right].
  /// Only applies when [title] is provided.
  final VitRadioPosition radioPosition;
  ...
  const VitRadio({
    ...
    this.titleStyle,
    this.subtitleStyle,
    this.radioPosition = VitRadioPosition.right,
    this.visualDensity,
    this.semanticLabel,
    this.hint,
    ...
    if (effectiveLoading) {
      ...
      return VitSkeletonShimmer(
        child: Container(
          ...
          child: Row(
            children: [
              if (radioPosition == VitRadioPosition.left) ...[
                Container(
                  width: 20,
                  height: 20,
                  decoration: BoxDecoration(
              ...
              if (radioPosition == VitRadioPosition.right) ...[
                const SizedBox(width: 16),
                Container(
                  width: 20,
                  height: 20,
    ...
    return Container(
      ...
      child: MergeSemantics(
        child: ListTileTheme.merge(
          ...
          child: ListTile(
            leading: radioPosition == VitRadioPosition.left
                ? radioWidget
                : icon,
            title: titleWidget,
            subtitle: subtitleWidget,
            trailing: radioPosition == VitRadioPosition.right
                ? radioWidget
                : icon,
            visualDensity: visualDensity,
...
/// A widget that groups multiple [VitRadio] buttons together.
///
/// [VitRadioGroup] simplifies managing multiple related radio options by
...
/// )
/// ```
class VitRadioGroup<T> extends StatefulWidget {
  ...
  final List<VitRadioOption<T>> options;
  ...
  ///
  /// Defaults to [VitRadioPosition.right].
  final VitRadioPosition radioPosition;
  ...
  @override
  State<VitRadioGroup<T>> createState() => _VitRadioGroupState<T>();
}

class _VitRadioGroupState<T> extends State<VitRadioGroup<T>> {
  T? _groupValue;
  ...
  @override
  void didUpdateWidget(VitRadioGroup<T> oldWidget) {
...
/// Configuration for a single radio option in a [VitRadioGroup].
///
/// Each [VitRadioOption] represents one selectable choice in the radio group.
class VitRadioOption<T> {
  ...
  /// Creates a [VitRadioOption].
  ///
  /// The [value] parameter is required.
  /// All other parameters are optional.
  const VitRadioOption({
    required this.value,
    this.title,
    this.subtitle,
    this.titleStyle,
