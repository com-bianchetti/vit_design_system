...
/// A highly customizable badge widget for indicating notifications, status, or counts in the Bit design system.
///
/// [BitBadge] can be used to display notification dots, numeric counts, icons, or custom labels. It is typically overlaid on top of another widget (such as an icon or avatar) to indicate new activity, status, or other contextual information.
///
/// Features:
...
/// - Integrates with [BitBadgeController] for dynamic updates
...
/// ```dart
/// final controller = BitBadgeController(count: 5);
/// ...
/// BitBadge(
...
class BitBadge extends StatefulWidget {
  const BitBadge({
    super.key,
  ...
  ///
  /// Defaults to the [BitTheme]'s visual density.
  ...
  /// part of the app.
  final BitBadgeController? controller;
  ...
  @override
  State<BitBadge> createState() => _BitBadgeState();
}

class _BitBadgeState extends State<BitBadge> {
  late BitBadgeController controller;

  @override
  void initState() {
    super.initState();
    controller = widget.controller ?? BitBadgeController();
